...
export default function WhatsAppInbox() {
  ...
  return (
    <div className="flex h-[100dvh] bg-background">
      ...
      <div className="w-16 flex flex-col items-center py-3 border-r shrink-0" style={{ background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(255,255,255,0.06)' }}>
        {/* Chats icon (active) */}
        <button
        ...
        {/* Placeholder icons */}
        <button className="w-10 h-10 rounded-xl flex items-center justify-center mb-2 transition-all hover:bg-white/10" style={{ color: 'rgba(255,255,255,0.5)' }} title="Status">
          <CheckCircle2 className="w-5 h-5" />
        </button>
        <button className="w-10 h-10 rounded-xl flex items-center justify-center mb-2 transition-all hover:bg-white/10" style={{ color: 'rgba(255,255,255,0.5)' }} title="New Chat" onClick={() => setShowNewConv(true)}>
          <MessageSquare className="w-5 h-5" />
        </button>
        {/* Spacer */}
        <div className="flex-1" />
        {/* Settings at bottom */}
        <button className="w-10 h-10 rounded-xl flex items-center justify-center transition-all hover:bg-white/10" style={{ color: 'rgba(255,255,255,0.5)' }} title="Settings" onClick={() => setActiveTab('settings')}>
          <Settings className="w-5 h-5" />
        </button>
      </div>

      {/* COLUMN 2: Chat List */}
      <div className={`${sidebarOpen ? 'w-[360px]' : 'w-0'} border-r flex flex-col shrink-0 transition-all duration-300 overflow-hidden`}>
        {/* Compact stats strip at top */}
        <div className="flex items-center justify-between px-3 py-1.5 text-[9px]" style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
          <span className="flex items-center gap-1" style={{ color: 'rgba(255,255,255,0.55)' }}>
            <span className="font-semibold" style={{ color: 'hsl(38 92% 50%)' }}>{conversationsToday}</span> Today
          </span>
          <span className="flex items-center gap-1" style={{ color: 'rgba(255,255,255,0.55)' }}>
            <span className="font-semibold" style={{ color: 'rgba(255,255,255,0.85)' }}>{avgResponseTime}m</span> Resp
          </span>
          <span className="flex items-center gap-1" style={{ color: 'rgba(255,255,255,0.55)' }}>
            <span className="font-semibold" style={{ color: 'rgba(255,255,255,0.85)' }}>{slaBreaches}</span> SLA
          </span>
          <span className="flex items-center gap-1" style={{ color: 'rgba(255,255,255,0.55)' }}>
            <span className="font-semibold" style={{ color: 'rgba(255,255,255,0.85)' }}>{unresolvedCount}</span> Open
          </span>
        </div>
