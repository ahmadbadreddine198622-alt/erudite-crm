...
export default function AIIntelligenceCard({ ai, analyzing, onReanalyse, collapsed, onToggle }) {
  ...
  return (
    <div style={css("flex:none; margin:0 16px 10px; border-radius:16px; border:1px solid hsl(38 92% 50% / 0.28); background:linear-gradient(180deg, hsl(38 92% 50% / 0.07), rgba(255,255,255,0.02)); overflow:hidden; animation: ld-rise 0.4s cubic-bezier(0.22,1,0.36,1) both;")}>
      ...
      {isCollapsed ? (
        <button onClick={onToggle} style={css("width:100%; display:flex; align-items:center; justify-content:space-between; gap:10px; padding:10px 15px; background:none; border:none; cursor:pointer; font-family:'Inter',sans-serif;")}>
        ...
        <React.Fragment>
          ...
          <div style={css("padding:14px 15px;")}>
            ...
            <div style={css("display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:10px; flex-wrap:wrap;")}>
              <button onClick={onToggle} style={css("display:inline-flex; align-items:center; gap:7px; background:none; border:none; cursor:pointer; font-family:'Inter',sans-serif; padding:0;")}>
                <span style={css("font-size:10.5px; font-weight:700; letter-spacing:0.07em; text-transform:uppercase; color:rgba(255,255,255,0.5);")}>AI Intelligence</span>
                <ChevronDown size={14} style={chevronStyle(false)} />
              </button>
            ...
            {hasNba && (
              <div style={css("margin-bottom:12px; padding:10px 13px; border-radius:11px; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); border-left:3px solid "+pMeta.color+";")}>
                <div style={css("display:flex; align-items:center; gap:8px; flex-wrap:wrap;")}>
                  ...
                  {priority && (
                    <span style={css("display:inline-flex; align-items:center; padding:2px 8px; borderRadius:99px; fontSize:9.5px; fontWeight:700; text-transform:uppercase; letter-spacing:0.04em; background:"+pMeta.bg+"; border:1px solid "+pMeta.border+"; color:"+pMeta.color+";")}>{priority}</span>
                  )}
                </div>
