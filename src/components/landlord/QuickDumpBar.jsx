...
// Agent dumps everything in free text (or speaks via mic), one tap on
// "AI Auto-Fill" and the LLM parses it into every structured field.
// Both paths (guided wizard + quick dump) write to the same form state.

const GOLD = 'hsl(38 92% 50%)';
const GOLD_DIM = 'rgba(250,180,40,0.12)';
...
export default function QuickDumpBar({ form, setForm, landlord }) {
...
You are an assistant inside a Dubai real estate CRM. An agent just finished a qualification call with ${ownerName}. Instead of clicking through questions, the agent dumped a freeform brain-dump of everything they learned. Parse that text and extract structured qualification data.

Return ONLY fields you can confidently identify from the text. For any field not mentioned, return null. Match enum values EXACTLY.
...
For freeform fields (motivation_notes, competing_brokers, next_step), extract verbatim or lightly cleaned.
For price_expectation_aed, extract the number (digits only).
For followup_date / available_from, extract as YYYY-MM-DD if mentioned, otherwise null.
...
${text}`;
      ...
      return base44.integrations.Core.InvokeLLM({
        ...
        response_json_schema: {
          type: 'object',
          properties: {
            motivation: { type: 'string' }, motivation_notes: { type: 'string' },
            timeline_urgency: { type: 'string' }, price_expectation_aed: { type: 'number' },
            price_vs_valuation: { type: 'string' }, mandate_openness: { type: 'string' },
            competing_brokers: { type: 'string' }, tenancy_status: { type: 'string' },
            available_from: { type: 'string' }, mortgage_status: { type: 'string' },
            is_decision_maker: { type: 'string' }, call_outcome: { type: 'string' },
            rapport_after_call: { type: 'string' }, next_step: { type: 'string' },
    ...
    onSuccess: data => {
      ...
      if (count === 0) { toast.error('Couldn\'t extract any fields from that text.'); return; }
      setForm(f => ({ ...f, ...updates }));
      setFilledCount(count);
      toast.success(`✅ AI filled ${count} field${count === 1 ? '' : 's'} — verify & save`);
    },
  ...
  const handleMic = async () => {
    if (vr.recording) {
      ...
      try {
        ...
        if (transcript) { setText(t => (t ? t + ' ' : '') + transcript + ' '); toast.success('🎤 Voice transcribed'); }
      } catch (e) { toast.error('Transcription failed: ' + (e?.message || 'error')); }
      finally { setTranscribing(false); }
    } else { vr.start(); }
  };

  return (
    <div className="rounded-xl border overflow-hidden" style={{ background: GOLD_DIM, borderColor: 'rgba(250,180,40,0.2)' }}>
      <div className="flex items-center gap-2 px-3 py-2 border-b" style={{ borderColor: 'rgba(250,180,40,0.1)' }}>
        <div className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ background: 'rgba(250,180,40,0.18)' }}>
          <Zap className="w-3.5 h-3.5" style={{ color: GOLD }} />
        </div>
        <span className="text-[11px] font-bold" style={{ color: GOLD }}>Quick Dump</span>
        <p className="text-[10px]" style={{ color: 'rgba(255,255,255,0.35)' }}>— or type/speak everything at once, AI fills the fields above</p>
        {filledCount > 0 && (
          <span className="text-[10px] font-semibold text-emerald-400 flex items-center gap-1 ml-auto">
      ...
      <div className="p-2.5">
        <textarea
          ...
          placeholder={"Type or speak freely… e.g.\n\"Owner relocating to London, wants 1.4M for 2BR in Marina. Mortgage free. Tenant leaves end of month. Open to exclusive. Sole owner on title. Interested, wants to move fast.\""}
          rows={3}
          className="w-full resize-none rounded-lg px-3 py-2 text-xs leading-relaxed"
          style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.9)', outline: 'none', minHeight: 64 }}
        ...
        {vr.error && <p className="text-[10px] text-red-400 px-2 mb-1">{vr.error}</p>}

        <div className="flex items-center gap-2 mt-1">
          <button
            onClick={handleMic}
            disabled={transcribing || autoFillMutation.isPending}
            className="flex items-center justify-center gap-1.5 h-9 px-3 rounded-lg transition-colors disabled:opacity-50 shrink-0"
            style={{
          ...
          >
            {transcribing ? <Loader2 className="w-4 h-4 animate-spin text-white/60" />
              : vr.recording ? <Square className="w-4 h-4 text-red-400" />
          ...
          <button
            ...
            style={{ background: `linear-gradient(180deg, ${GOLD}, hsl(38 92% 46%))`, color: '#1a1205', border: '1px solid rgba(250,180,40,0.5)' }}
          >
          ...
          {hasText && !autoFillMutation.isPending && (
            <button
              onClick={() => { setText(''); setFilledCount(0); }}
              className="px-3 h-9 rounded-lg text-xs font-medium transition-colors shrink-0"
