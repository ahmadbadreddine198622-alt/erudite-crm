...
/**
 * Twilio outbound call dialog.
 * Server-side call via twilioMakeCall — Twilio dials the customer directly.
 * Agent can edit the number before calling.
 */
export default function TwilioCallDialog({ lead, contact, iconOnly = false }) {
  ...
  // phase: idle | calling | active | ended
  ...
  const handleCall = async () => {
    ...
    try {
      ...
      if (res.data?.ok) {
        ...
        setErrorMsg(res.data?.error || 'Call failed');
        setPhase('idle');
      }
    } catch (err) {
      ...
      >
        ...
        {phase === 'idle' && (
          <div className="p-6 space-y-4">
            ...
            {/* Editable number — always shown so user can call a different number */}
            <div>
              ...
              {defaultPhone && dialNumber !== defaultPhone && (
                <button
                  onClick={() => setDialNumber(defaultPhone)}
          ...
          <div className="flex flex-col" style={{ background: '#0a1628' }}>
            <div className="flex flex-col items-center pt-8 pb-4 px-6 gap-2">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px] uppercase tracking-widest font-bold text-green-400">Call Connected</span>
              </div>
              ...
              <p className="text-xs mt-1 text-center" style={{ color: 'rgba(255,255,255,0.3)' }}>
                Call is running on the customer's phone. Hang up from here when done.
              </p>
            </div>
