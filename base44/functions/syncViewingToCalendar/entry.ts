...
// Syncs a Viewing record to Google Calendar (agent as organizer, landlord as attendee so it
...
Deno.serve(async (req) => {
  try {
    ...
    const viewing = data;
    const viewingId = event.entity_id;

    if (!viewing || !viewing.scheduled_at) {
    ...
    let landlord = null;
    if (viewing.landlord_id) {
      landlord = await base44.asServiceRole.entities.Landlord.get(viewing.landlord_id).catch(() => null);
    }
    const landlordName = landlord?.full_name_en || landlord?.full_name_ar || 'Client';
    const landlordEmail = landlord?.email || null;
    const agentEmail = viewing.agent_email || landlord?.assigned_agent_email || null;

    const { accessToken } = await base44.asServiceRole.connectors.getConnection('googlecalendar');

    const startMs = new Date(viewing.scheduled_at).getTime();
    const durationMin = Number(viewing.duration_minutes) > 0 ? Number(viewing.duration_minutes) : 30;
    ...
    const event_data = {
      summary: viewing.title || `Property Viewing — ${landlordName}`,
      description: [
        `Client: ${landlordName}`,
        `Agent: ${agentEmail || 'N/A'}`,
        `Location: ${viewing.location || 'N/A'}`,
    ...
    const existingEventId = viewing.google_event_id || null;
    ...
    if (calendarEventId && calendarEventId !== existingEventId) {
      await base44.asServiceRole.entities.Viewing.update(viewingId, { google_event_id: calendarEventId });
    }

    // Send a confirmation email to the landlord (once, on first creation)
    if (landlordEmail && calendarAction === 'created' && !viewing.confirmation_email_sent) {
      const dateFormatted = new Date(viewing.scheduled_at).toLocaleString('en-GB', {
        timeZone: 'Asia/Dubai', weekday: 'short', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit',
      });
      await base44.asServiceRole.integrations.Core.SendEmail({
        to: landlordEmail,
        subject: `Please confirm your viewing — ${dateFormatted}`,
        body: `Hi ${landlordName},\n\nWe've scheduled a property viewing for you:\n\n📅 ${dateFormatted}\n📍 ${viewing.location || 'N/A'}\n\nPlease reply to confirm you're available, or let us know if you'd like to reschedule.\n\nLooking forward to seeing you.`,
      }).catch(() => null);
      await base44.asServiceRole.entities.Viewing.update(viewingId, { confirmation_email_sent: true });
    }
