// parseFormF — DLD Unified Sell Contract (F) parser
// Mirrors parseFormA engine: unpdf text extraction + section-anchored regex.
// NO LLM, NO external API, NO env secrets. Confirm-first write gate.
// Write target: Lead (not Landlord). No 4-tier matching — caller passes lead_id.
//
// Tuned against a real signed Form F (CF202605218454, Binghatti Gate).
// Bilingual PDFs interleave Arabic/English on the same line, so all field
// regexes are scoped to carved section blocks via between() to avoid decoys
// (e.g. Sell Price shares a line with Deposit Amount; buyer name appears
// reversed in the National ID block).
//
// SDK pattern mirrors parseFormA exactly: createClientFromRequest(req),
// base44.asServiceRole.entities, and Entity.filter({id})[0] for reads.
...
// ---- the extraction core ----
function parseFormF(raw: string) {
...
Deno.serve(async (req) => {
  try {
    // SDK init: mirror parseFormA exactly — createClientFromRequest, then
    // a softened auth.me() gate (no role check — per-lead context is the
    // gate, not admin role).
    ...
    // seller + date appended to notes (composed, applied at write time so we can
    // merge with existing notes rather than overwrite)
    const noteLine =
    ...
    try {
      // Service-role accessor + filter({id})[0] read pattern — mirrors parseFormA.
      const svc = base44.asServiceRole.entities;
      const leads = await svc.Lead.filter({ id: lead_id });
      const lead = leads && leads[0];
      if (!lead) {
        return Response.json({ ...preview, mode: 'refused', reason: 'lead_not_found', lead_id });
      }

      const patch: Record<string, any> = { ...(preview.proposed_lead_updates || {}) };
