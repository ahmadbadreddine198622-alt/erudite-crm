 ...
 * matching LandlordProperty records.
 *
 * Input: { market_report_id }
 *
 * Verified live schema (do NOT touch entity schemas):
 *  - MarketReport: project_name, report_file_url, market_event_date (default 2026-02-28),
 *    transactions_count, median_price_aed, median_price_sqft,
 *    median_price_sqft_pre_event, median_price_sqft_post_event, analysis_summary,
 *    status [uploaded|extracting|analyzed|failed]
 *  - MarketTransaction: market_report_id, project_name, unit_number, transaction_date,
 *    price_aed, area_sqft, price_per_sqft, bedrooms [studio|1br|2br|3br|4plus],
 *    sale_status [offplan|ready], is_post_event
 *  - LandlordProperty: landlord_id, property_id, ai_estimated_value_aed,
 *    ai_estimated_price_sqft, ai_valuation_confidence [high|medium|low],
 *    ai_valuation_basis, ai_valuation_updated_at
 *  - Property (via property_id): area_sqft (number), bedrooms (number), property_type
 *  - Landlord: project_name  ← valuation match key (case-insensitive)
 *
 * NOTE: MarketReport has overall + pre/post price-per-sqft medians but NO
 * per-bedroom median fields, so per-bedroom medians are computed for valuation and
 * folded into analysis_summary (text) rather than persisted as structured fields.
 *
...
// base64-encode an ArrayBuffer in chunks (avoids call-stack overflow on big PDFs)
...
// Property.bedrooms is a NUMBER; property_type may be 'studio'
...
// weighted median over [{value, weight}] — expand by integer weight, then median
...
Return ONLY a strict JSON array — no prose, no markdown, no code fences. Each element is one transaction row:
{
  ...
  "price_aed": number,            // total price in AED, digits only (no commas, no "AED")
  "area_sqft": number,            // size in square feet, digits only
  "price_per_sqft": number,       // AED per sqft; if not printed, compute price_aed / area_sqft
...
async function callClaude(apiKey, base64Pdf) {
  const resp = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model: MODEL,
      max_tokens: 16000, // NOTE: very large reports may truncate; see stop_reason handling
      // Opus 4.8: no temperature/top_p/top_k/budget_tokens (those 400).
      messages: [{
        role: 'user',
        content: [
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // idempotency: clear prior rows for this report
    await deleteExistingTransactions(svc, reportId);

    // 1) fetch + base64 the PDF
    ...
    // 3) build + persist MarketTransactions
    const txs = [];
    for (const r of rows) {
      ...
      if (price_aed == null && area_sqft == null) continue; // unusable row
    ...
    // per-bedroom medians (for valuation + summary; not persisted as fields)
    const perBed = {};
    ...
    // 5) valuation fan-out → LandlordProperty whose Landlord.project_name matches
    ...
    if (wantProject && txs.length) {
      ...
      for (const landlordId of matchLandlordIds) {
        ...
        for (const lp of lps) {
          ...
          if (!matched.length) matched = bedComps; // area filter emptied → fall back to bedroom match
          ...
          if (postSameBed.length >= 1) {
            ...
            // zero post-event comps for this bedroom type → building post/pre shift (low confidence)
